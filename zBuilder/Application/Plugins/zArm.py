...
def XSILoadPlugin(in_reg):
	...
	in_reg.RegisterCommand('zArm', 'zArm')
...
class zArm(object):
	...
	def template():
		...
		def fget(self):
			...
			if not self._template:
				# wrap a new class #
				self._template = win32com.server.util.wrap(zArm_Template(self))
	...
	def rig():
		...
		def fget(self):
			...
			if not self._rig:
				# wrap a new class #
				self._rig = win32com.server.util.wrap(zArm_Rig(self))
...
class zArm_Template(object):
	"""docstring for zArm_Template"""
	...
	def __init__(self, parent):
		super(zArm_Template, self).__init__()
...
class zArm_Rig(object):
	...
	def Build(self):
		...
		# build the start transform #
		trans_start = self.root_skel_arm.Bones(1).Kinematics.Global.Transform
		# trans_start.AddLocalRotation(XSIMath.CreateRotation(0, 0, XSIMath.DegreesToRadians(-90)))
		...
		ribbon_lower.node_start			= self.root_skel_arm.Bones(1)
		...
		# # remove all but the center deformer #
		# del_col = dispatch('XSI.Collection')
		# for node in ribbon_lower.ribbon_branch.FindChildren('*_Env'):
		# 	if re.match(r'^.+[1245]_.+$', node.Name, re.I):
		# 		del_col.Add(node)
		# xsi.DeleteObj(del_col)
