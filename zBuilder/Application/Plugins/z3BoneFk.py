...
def XSILoadPlugin(in_reg):
	...
	in_reg.Name = "z3BoneFk"
	in_reg.Email = "andy@zoogloo.net"
	in_reg.URL = ""
	in_reg.Major = 1
	in_reg.Minor = 0

	in_reg.RegisterCommand('z3BoneFk', 'z3BoneFk')
...
class z3BoneFk(object):
	...
	def template():
		...
		def fget(self):
			...
			if not self._template:
				# wrap a new class #
				self._template = win32com.server.util.wrap(z3BoneFk_Template(self))
	...
	def rig():
		...
		def fget(self):
			...
			if not self._rig:
				# wrap a new class #
				self._rig = win32com.server.util.wrap(z3BoneFk_Rig(self))
...
class z3BoneFk_Template(object):
	"""docstring for z3BoneFk_Template"""
	...
	def __init__(self, parent):
		super(z3BoneFk_Template, self).__init__()
	...
	def Draw(self):
		...
		node_joint2	= node_parent.AddNull(
			xsi.zMapName(
				'%sJoint2' % self.parent.basename, 
				'Custom:Tmp', 
				self.parent.symmetry
			)
		)
		node_joint2.AddProperty('CustomProperty', False, 'zBuilderTemplateManip')
		node_joint2.AddProperty('CustomProperty', False, 'z%sJoint2' % self.parent.basename)
		...
		# wrist #
		v_result.Scale(self.parent.scale, self.v_joint2)
		trans.Translation = v_result
		node_joint2.Kinematics.Global.Transform = trans
		...
		cns = node_joint1.Kinematics.AddConstraint('Direction', node_joint2, False)
		cns = dispatch(cns)

		cns = node_joint2.Kinematics.AddConstraint('Direction', node_tip, False)
		cns = dispatch(cns)

		trans = node_tip.Kinematics.Global.Transform
		trans.Rotation = node_joint2.Kinematics.Global.Transform.Rotation
		node_tip.Kinematics.Global.Transform = trans
	...
	def GetFromScene(self):
		...
		if not node_parent:
			raise Exception('Unable to find template container by id: %s and name: %s (%s)' % (self.parent.uid, self.parent.basename, self.parent.symmetry))
		...
		for node in child_nodes:
			...
			elif node.Properties('z%sJoint2' % self.parent.basename):
				self.v_joint2		= node.Kinematics.Global.Transform.Translation
				self.t_joint2		= node.Kinematics.Global.Transform
...
class z3BoneFk_Rig(object):
	...
	def __init__(self, parent):
		super(z3BoneFk_Rig, self).__init__()
	...
	def Build(self):
		...
		if not self.skeleton_parent:
			raise Exception(
				'skeleton_parent is not defined.'
			)
		...
		if not self.controls_parent:
			raise Exception(
				'controls_parent is not defined.'
			)
		...
		if not self.deformer_parent:
			raise Exception(
				'deformer_parent is not defined.'
			)
		...
		self.root_con.AddBone(
			template.v_tip,
			c.siChainBonePin,
			xsi.zMapName('%s' % self.parent.basename, 'Control', self.parent.symmetry, 2, self.inc_alpha)
		)
		...
		if self.parent.template.use_transforms:
			...
			temp_null.Kinematics.Global.Transform = self.parent.template.t_joint2
			xsi.MatchTransform(self.root_con.Bones(2), temp_null, c.siRot, False)
		...
		self.root_skel.AddBone(
			template.v_tip,
			c.siChainBonePin,
			xsi.zMapName('%s' % self.parent.basename, 'ChainBone', self.parent.symmetry, 2, self.inc_alpha)
		)
