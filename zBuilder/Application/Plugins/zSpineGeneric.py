...
Effort to make the spine more generic.  Previous one relied on presets.
...
def XSILoadPlugin(in_reg):
	...
	in_reg.Name = "zSpineGeneric"
	in_reg.Email = "andy@zoogloo.net"
	in_reg.URL = ""
	in_reg.Major = 1
	in_reg.Minor = 0

	in_reg.RegisterProperty('zSpineGenericCurve')

	in_reg.RegisterCommand('zSpineGeneric', 'zSpineGeneric')
...
def zSpineGenericCurve_Define(ctxt):
...
def zSpineGenericCurve_DefineLayout(ctxt):
	lo = ctxt.Source
	lo.Clear()
...
class zSpineGeneric(object):
	...
	uid				= '314b14e56b739f0de7e7214476c60cfc'
	
	def __init__(self, basename='Spine'):
		super(zSpineGeneric, self).__init__()
	...
	def template():
		...
		def fget(self):
			...
			if not self._template:
				# wrap a new class #
				self._template = dispatch(win32com.server.util.wrap(zSpineGeneric_Template(self)))
...
class zSpineGeneric_Template(object):
	"""docstring for zSpineGeneric_Template"""
	# required for COM wrapper #
	_public_methods_ = [
		'Draw',
		'LoadDefaultValues',
	...
	def __init__(self, parent):
		super(zSpineGeneric_Template, self).__init__()
	...
	def LoadDefaultValues(self):
		...
		# set the defaults #
		if self.parent.basename in defaults.keys():
	...
	def GetFromScene(self):
		...
		if not node_parent:
			raise Exception('Unable to find pelvis template container by id: %s and name: %s (%s)' % (self.parent.uid, self.parent.basename, self.parent.symmetry))
		...
		for node in child_nodes:
			prop = node.Properties('zSpineGenericCurve')
			if prop:
				self.curve 	= node
				self.prop 	= prop
		...
		# post conditions #	
		if not self.curve:
			raise Exception('spine.template.curve is undefinfed.')
		if not self.prop:
			raise Exception('spine.template.prop is undefinfed.')
		
class zSpineGeneric_Rig(object):
	...
	def Build(self):
		...
		if not self.skeleton_parent:
			raise Exception(
				'zSpineGeneric.rig.skeleton_parent is not defined.'
			)
		...
		if not self.controls_parent:
			raise Exception(
				'zSpineGeneric.rig.controls_parent is not defined.'
			)
		...
		if not self.deformer_parent:
			raise Exception(
				'zSpineGeneric.rig.deformer_parent is not defined.'
			)
		...
		if not template.curve:
			raise Exception(
				'Missing template curve.  Try using zSpineGeneric.template.Draw()'
			)
		
		# make sure the curve has the property #
		prop_spine = template.curve.Properties('zSpineGenericCurve')
		if not prop_spine:
			raise Exception(
				'Property "zSpineGenericCurve" not found on "%s"' % template.curve
			)
		...
		self.controls_parent 	= dispatch(self.controls_parent)
		dnt_node 	= self.do_not_touch.AddNull(xsi.zMapName(self.parent.basename, 'Custom:DoNotTouch', self.parent.symmetry))
		con_node 	= self.controls_parent.AddNull(xsi.zMapName(self.parent.basename, 'Custom:Bunch', self.parent.symmetry))
		...
		if self.character_set:
			...
			# add the ik cons #
			for con in self.con_iks:
				self.character_subset.AddNodePosRot(con.node_con)
		...
		if self.add_chest_con:
			...
			# add the chest con to the character set #
		...
		if self.add_head_con:
			...
			# add the chest con to the character set #
			self.character_subset.AddNodePosRot(self.con_head.node_con)		# # for setup purposes only #
...
def zSpineGeneric_Execute(basename):
	# export the python object #
	import win32com.server
	return win32com.server.util.wrap(
		zSpineGeneric(basename)
	)
