...
__date__    = '$Date: 2009-06-22 16:48 -0700 $'
...
def XSILoadPlugin(in_reg):
	...
	in_reg.RegisterCommand('zAnimalLeg_1_1', 'zAnimalLeg_1_1')
...
class zAnimalLeg_Template(object):
	...
	def LoadDefaultValues(self):
		...
		self.v_knee				= XSIMath.CreateVector3(4.5317, 12.542, 4.7761)
		# self.v_hawk				= XSIMath.CreateVector3(3.2317, 7.811, -3.4628)
		self.v_hawk				= XSIMath.CreateVector3(5.5317, 7.811, -3.4628)
		self.v_ankle			= XSIMath.CreateVector3(5.5317, 2.5095, -0.0658)
		self.v_toe				= XSIMath.CreateVector3(6.317, 2.2571, 1.8452)
...
class zAnimalLeg_Rig(object):
	...
	def Build(self):
		...
		#---------------------------------------------------------------------
		# reorient the foot to match the controller #
		if self.reorient_foot_con:
			
			# get the foot vector #
			v_foot 		= XSIMath.CreateVector3()
			v_ankle 	= self.root_skel_foot.Kinematics.Global.Transform.Translation
			v_toe	 	= self.root_skel_foot.Effector.Kinematics.Global.Transform.Translation
			v_foot.Sub(v_toe, v_ankle)
			
			# flatten the Y component #
			v_foot.Y 	= 0
			
			# get a forward Z vector and calculate the angle #
			v_forward 	= XSIMath.CreateVector3(0, 0, 1)
			angle		= v_forward.Angle(v_foot)
			log(XSIMath.RadiansToDegrees(angle))
			
			# move the hooke node to the scene root temporarily #
			xsi.ActiveSceneRoot.AddChild(self.con_foot.node_hook)
			
			# add the rotation offset #
			trans = self.con_foot.node_rest.Kinematics.Global.Transform
			trans.AddLocalRotation(
				XSIMath.CreateRotation(0, angle, 0)
			)
			self.con_foot.node_rest.Kinematics.Global.Transform = trans
			
			# move the hook node back under the con #
			self.con_foot.node_con.AddChild(self.con_foot.node_hook)
			
		#---------------------------------------------------------------------
		# create a hawk roation controller #
		self.con_hawk 						= xsi.zCon()
		self.con_hawk.type 					= 'rot'
		self.con_hawk.size 					= self.size_hawk_con * self.parent.scale
		self.con_hawk.transform 			= self.con_foot.node_con.Kinematics.Global.Transform
		self.con_hawk.basename 				= 'Hawk'
		self.con_hawk.symmetry 				= self.parent.symmetry
		self.con_hawk.parent_node 			= self.con_foot.node_hook
		self.con_hawk.rotation_order 		= 'zyx'
		if re.match(r'^right$', self.parent.symmetry, re.I):
			self.con_hawk.red 				= 0.8
			self.con_hawk.green 			= 0
			self.con_hawk.blue 				= 0
		else:                               	
			self.con_hawk.red 				= 0
			self.con_hawk.green 			= 0.8
			self.con_hawk.blue 				= 0
		self.con_hawk.Draw()
		# self.con_hawk.AddTransformSetupRot('local', False, False, True) # only z axis
		self.con_hawk.AddTransformSetupRot('local')
		
		# get the offset to the hawk #
		v_result = XSIMath.CreateVector3()
		v_result.Sub(
			self.root_con.Bones(2).Kinematics.Global.Transform.Translation,
			self.root_con.Effector.Kinematics.Global.Transform.Translation
		)
		# make it a little longer than the joint itself #
		v_result.ScaleInPlace(1.25)

		# get the rotation angle #
		v_angle = XSIMath.CreateVector3(0, 1, 0)
		angle = XSIMath.RadiansToDegrees(v_result.Angle(v_angle))
		self.con_hawk.Rotate(0, 0, angle)

		# offset the con #
		trans = self.con_hawk.node_con.Kinematics.Global.Transform
		trans.Translation = self.root_con.Bones(2).Kinematics.Global.Transform.Translation
		m4 = trans.Matrix4
		# log('%s %s %s %s\r\n%s %s %s %s\r\n%s %s %s %s\r\n%s %s %s %s\r\n' % (
		# 	m4.Value(0, 0), m4.Value(0, 1), m4.Value(0, 2), m4.Value(0, 3), 
		# 	m4.Value(1, 0), m4.Value(1, 1), m4.Value(1, 2), m4.Value(1, 3), 
		# 	m4.Value(2, 0), m4.Value(2, 1), m4.Value(2, 2), m4.Value(2, 3), 
		# 	m4.Value(3, 0), m4.Value(3, 1), m4.Value(3, 2), m4.Value(3, 3) 
		# ))
		self.con_hawk.Xform(
			m4.Value(0, 0), m4.Value(0, 1), m4.Value(0, 2), m4.Value(0, 3), 
			m4.Value(1, 0), m4.Value(1, 1), m4.Value(1, 2), m4.Value(1, 3), 
		...
		# add a node to constrain the hawk to #
		node_hawk_cns = self.con_hawk.node_hook.AddNull(
			xsi.zMapName('Hawk', 'Custom:Cns', self.parent.symmetry)
		)
		node_hawk_cns.Kinematics.Global.Transform = self.root_skel.Bones(2).Kinematics.Global.Transform
		xsi.zHide(node_hawk_cns)
		
		# add constraints to the leg #
		cns_hawk = self.root_con.Bones(2).Kinematics.AddConstraint('Position', node_hawk_cns, True)
		# cns_hawk.blendweight.AddExpression(self.ik_switch.FullName)
		self.con_hawk.node_con.Properties('Visibility').viewvis.AddExpression(
			'cond(%s.chain.blendik != 1, 0, 1)' % self.root_con.Bones(0).FullName
		)
		...
		if self.character_set:
			...
			self.character_subset.AddNodeRot(self.con_hawk.node_con)
...
def zAnimalLeg_1_1_Execute(basename, symmetry):
