...
Essentiall the same as zSpine but with awareness of symmetry.

Uses zSpineCurve property from zSpine.

Created by andy on 2009-03-03.
...
def XSILoadPlugin(in_reg):
	...
	in_reg.Name = "zSpine_2_0"
	in_reg.Email = "andy@zoogloo.net"
	in_reg.URL = ""
	in_reg.Major = 1
	in_reg.Minor = 0

	in_reg.RegisterCommand('zSpine_2_0', 'zSpine_2_0')
...
class zSpine_2_0(object):
	...
	uid				= 'bee65e36c56438b64ef2197cac88bc6b'
	
	def __init__(self, basename='Spine', symmetry='middle', preset='spine'):
		super(zSpine_2_0, self).__init__()
	...
	def template():
		...
		def fget(self):
			...
			if not self._template:
				# wrap a new class #
				self._template = dispatch(win32com.server.util.wrap(zSpine_2_0_Template(self)))
	...
	def rig():
		...
		def fget(self):
			...
			if not self._rig:
				# wrap a new class #
				self._rig = dispatch(win32com.server.util.wrap(zSpine_2_0_Rig(self)))
...
class zSpine_2_0_Template(object):
	"""docstring for zSpine_2_0_Template"""
	# required for COM wrapper #
	_public_methods_ = [
		'Draw',
		'LoadDefaultValues',
	...
	def __init__(self, parent):
		super(zSpine_2_0_Template, self).__init__()
	...
	def LoadDefaultValues(self):
		...
		# set the defaults #
		if not self.parent.preset in defaults.keys():
			raise Exception(
				'What an ass, there is no curve preset named "%s"' % self.parent.preset
			)
		self.point_array	= defaults.get(self.parent.preset).get('points')
		self.segments		= defaults.get(self.parent.preset).get('segments')
	...
	def Draw(self):
		...
		self.curve.Name = '%s_%s_Curve' % (self.parent.basename, self.parent.symmetry[0].upper())
	...
	def GetFromScene(self):
		...
		for node in dispatch(self.model).FindChildren('*'):
			if node.Properties('zContainer'):
				...
				and re.match(node.Properties('zContainer').Parameters('ContainerSym').Value, self.parent.symmetry, re.I) \
				and	re.match(node.Properties('zContainer').Parameters('ContainerName').Value, self.parent.basename, re.I):
					node_parent = node
					break
		# make sure we have the container #
		if not node_parent:
			raise Exception('Unable to find spine template container by id: %s and name: %s_%s_Container' % (self.parent.uid, self.parent.basename, self.parent.symmetry.lower()))
...
class zSpine_2_0_Rig(object):
	...
	def __init__(self, parent):
		super(zSpine_2_0_Rig, self).__init__()
		...
		self.controls_cns_usepose	= False
		...
		# try to get the template from the scene #
		try:
			self.parent.template.GetFromScene()
		except:
			# template doesn't exist #
			# since we need the spine curve, lets draw one #
			self.parent.template.Draw()
			# now lets get the values from the scene #
			self.parent.template.GetFromScene()
	...
	def Build(self):
		...
		if not template.curve:
			raise Exception(
				'Missing template curve.  Try using zSpine_2_0.template.Draw()'
			)
		...
		dnt_node 	= self.do_not_touch.AddNull('%s_%s_DoNotTouch' % (self.parent.basename, self.parent.symmetry[0].upper()))
		con_node 	= self.controls_parent.AddNull('%s_%s_Bunch' % (self.parent.basename, self.parent.symmetry[0].upper()))
		...
		control_curve.Name = '%s_%s_SourceCurve' % (self.parent.basename, self.parent.symmetry[0].upper())
		self.curve_control = control_curve
		
		shrink_curve = dnt_node.AddNurbsCurve(point_list2)
		shrink_curve.Name = '%s_%s_ShrunkCurve' % (self.parent.basename, self.parent.symmetry[0].upper())
		...
		for p in xrange(control_geom.Points.Count):
			...
			# create the con #  
			# conStack = xsi.zCreateCon(con_node, '%sIk%s' % (basename, p), self.parent.symmetry, 0, 2*conSize, 1, 1, 0)
			...
			con_ik.symmetry 		= self.parent.symmetry
		...
		guide_curve.Name = '%s_%s_CurveHull' % (self.parent.basename, self.parent.symmetry[0].upper())
		guide_curve.Properties('Visibility').selectability.Value = False
		
		# create center at each cluster #
		guide_clusters = dnt_node.AddNull('%s_%s_HullClusters' % (self.parent.basename, self.parent.symmetry[0].upper()))
		...
		for p in xrange(guide_curve.ActivePrimitive.Geometry.Points.Count):
			# create the cluster #
			cluster = guide_curve.ActivePrimitive.Geometry.AddCluster(c.siVertexCluster, '%sHull%s_%s_Cls' % (self.parent.basename, p, self.parent.symmetry[0].upper()), [p])

			# create a cluster center #
			cluster_center = guide_clusters.AddNull('%s%s_%s_HullClsCenter' % (self.parent.basename, p, self.parent.symmetry[0].upper()))
		...
		for i in xrange(segments+1):
			...
			if i == 0:
				...
				if not self.root_skel:
					...
					# rename #
					self.root_ik.Name 			= xsi.zMapName('%sIk' % self.parent.basename, 'ChainRoot', self.parent.symmetry)
					self.root_ik.Bones(0).Name 	= xsi.zMapName('%sIk' % self.parent.basename, 'ChainBone', self.parent.symmetry, 1)
					self.root_ik.effector.Name 	= xsi.zMapName('%sIk' % self.parent.basename, 'ChainEff', self.parent.symmetry)
					...
					# rename #
					self.root_fk.Name 			= xsi.zMapName('%sFk' % self.parent.basename, 'ChainRoot', self.parent.symmetry)
					self.root_fk.Bones(0).Name 	= xsi.zMapName('%sFk' % self.parent.basename, 'Control', self.parent.symmetry, 1)
					self.root_fk.effector.Name 	= xsi.zMapName('%sFk' % self.parent.basename, 'ChainEff', self.parent.symmetry)
					...
					self.root_skel.Bones(0).Name 	= xsi.zMapName(self.parent.basename, 'ChainBone', self.parent.symmetry, 1)
				...
				# all other bones #
					...
					bone_ik.Name 	= xsi.zMapName('%sIk' % self.parent.basename, 'ChainBone', self.parent.symmetry, segment_number)
					bone_fk.Name 	= xsi.zMapName('%sFk' % self.parent.basename, 'Control', self.parent.symmetry, segment_number)
					bone_skel.Name 	= xsi.zMapName('%s' % self.parent.basename, 'ChainBone', self.parent.symmetry, segment_number)
		...
		for i in xrange(self.root_ik.Bones.Count):
			...
			twist = parent.AddNull(xsi.zMapName(self.parent.basename, 'Custom:Twist', self.parent.symmetry, i))
		...
		#-------------------------------------------------------------------------
		# change the rotation order on the skeleton
		for i in xrange(self.root_skel.Bones.Count):
			# set the rotation order #
			ro = 0
			if   re.match(r'^xyz$', self.fk_rotation_order, re.I): 		ro = 0
			elif re.match(r'^xzy$', self.fk_rotation_order, re.I): 		ro = 1
			elif re.match(r'^yxz$', self.fk_rotation_order, re.I): 		ro = 2
			elif re.match(r'^yzx$', self.fk_rotation_order, re.I): 		ro = 3
			elif re.match(r'^zxy$', self.fk_rotation_order, re.I): 		ro = 4
			elif re.match(r'^zyx$', self.fk_rotation_order, re.I): 		ro = 5
			self.root_fk.Bones(i).Kinematics.Local.Parameters('rotorder').Value = ro
		...
		for b in xrange(self.root_skel.Bones.Count):
			...
			# create the nodes #
			node_dfm_parent = self.deformer_parent.AddNull(xsi.zMapName('%s' % self.parent.basename, 'Custom:DfmPrnt', self.parent.symmetry, b))
			node_dfm_shadow = node_dfm_parent.AddNull(xsi.zMapName('%s' % self.parent.basename, 'Custom:DfmShdw', self.parent.symmetry, b))
			env   			= node_dfm_shadow.AddNull(xsi.zMapName('%s' % self.parent.basename, 'Env', self.parent.symmetry, b))
		...
		if self.add_chest_con:
			...
			self.con_chest.symmetry 				= self.parent.symmetry
...
def zSpine_2_0_Init(ctxt):
	...
	oArgs.Add('symmetry', c.siArgumentInput, 'middle', c.siString)
	oArgs.Add('preset', c.siArgumentInput, 'spine', c.siString)
	#oArgs.AddObjectArgument('model')

	return True
	
def zSpine_2_0_Execute(basename, symmetry, preset):
	# export the python object #
	import win32com.server
	return win32com.server.util.wrap(
		zSpine_2_0(basename, symmetry, preset)
	)
