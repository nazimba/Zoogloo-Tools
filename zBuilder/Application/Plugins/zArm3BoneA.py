...
__date__    = '$Date: 2010-03-28 10:03 -0700 $'
...
def XSILoadPlugin(in_reg):
	...
	in_reg.Name = "zArm3BoneA"
	in_reg.Email = "andy@zoogloo.net"
	in_reg.URL = ""
	in_reg.Major = 1
	in_reg.Minor = 1

	in_reg.RegisterCommand('zArm3BoneA', 'zArm3BoneA')
...
class zArm3BoneA(object):
	...
	def __init__(self, sym='left'):
		super(zArm3BoneA, self).__init__()
	...
	def template():
		...
		def fget(self):
			...
			if not self._template:
				# wrap a new class #
				self._template = win32com.server.util.wrap(zArm3BoneA_Template(self))
	...
	def rig():
		...
		def fget(self):
			...
			if not self._rig:
				# wrap a new class #
				self._rig = win32com.server.util.wrap(zArm3BoneA_Rig(self))
...
class zArm3BoneA_Template(object):
	"""docstring for zArm3BoneA_Template"""
	...
	def __init__(self, parent):
		super(zArm3BoneA_Template, self).__init__()
...
class zArm3BoneA_Rig(object):
	...
	def Build(self):
		...
		if self.shrug_fk_fix:
			...
			cns = self.root_arm_con.Kinematics.AddConstraint('Pose', self.root_skel_spine.effector, True)
			cns = dispatch(cns)
		...
		#---------------------------------------------------------------------
		# align the chain to the template #
		if self.align_hand_to_template:
			log('-->' + template.model.Name)
			node_hand = template.model.FindChild('ArmWrist_%s_Tmp' % self.parent.symmetry[0].upper()) 
			# create a transform of just rotation and translation #
			tfm_hand = XSIMath.CreateTransform()
			tfm_hand.Translation = node_hand.Kinematics.Global.Transform.Translation
			tfm_hand.Rotation = node_hand.Kinematics.Global.Transform.Rotation
			
			self.root_hand_con.Kinematics.Global.Transform  = tfm_hand
			self.root_skel_hand.Kinematics.Global.Transform = tfm_hand
		...
		ribbon_upper.node_end			= self.root_skel_arm.Bones(1)
		...
		ribbon_lower.node_start			= self.root_skel_arm.Bones(2)
...
def zArm3BoneA_Execute(symmetry):
